...
export default function CrowdMeter({
  ...
  try {
    ...
    const response = await fetch(`/api/crowd?${params.toString()}`);
    const payload = await response.json();

    if (!response.ok || payload.error) {
      throw new Error(payload.error || "Crowd data unavailable");
    }

    setData(payload);
    setErr(null);
  } catch (error: any) {
    setErr(error.message || "Crowd data unavailable");
  }
}
  ...
  }, [city, attraction, popularity, bestTime, scope]);
...
if (err) return <div className="text-xs text-red-500 p-4">Crowd unavailable</div>;
if (!data) return <div className="text-xs text-gray-400 p-4">Loading crowd…</div>;
...
const weather = data.weather || { condition: "—", temperature: 0, humidity: 0 };
const updatedAt = data.updatedat ? new Date(data.updatedat * 1000).toLocaleTimeString() : "—";

return (
  <div className="rounded-2xl border border-gray-200 p-4 bg-white shadow-sm w-full">
    <div className="flex items-center justify-between mb-2">
      <div className="text-sm font-semibold text-gray-700">
        {scope === "city" ? `${city} Overall Crowd` : "Live Crowd"}
      </div>
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        <div className="flex justify-between text-xs mb-1">
          <span className="font-medium" style={{ color }}>{data.level || "Low"}</span>
    ...
    {reasons.length > 0 && (
      <div className="mt-3">
        <div className="text-xs uppercase tracking-wide text-gray-400 mb-1">Why</div>
        <div className="flex flex-wrap gap-1.5">
          {reasons.map((r) => (
            <span key={r} className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-700">{r}</span>
          ))}
        </div>
      </div>
    )}
    ...
    {data.advice && (
      <div className="mt-3 text-xs text-gray-600 flex gap-1.5">
        <span>💡</span><span>{data.advice}</span>
      </div>
    )}
