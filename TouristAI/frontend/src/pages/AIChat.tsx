...
const getScheduleSectionMeta = (label: string) => {
  ...
  if (normalized.includes("shopping") || normalized.includes("free time")) return { icon: Compass, tone: "text-fuchsia-500 bg-fuchsia-50 dark:bg-fuchsia-950/20", label };
...
const parseScheduleHeading = (line: string): { label: string; time?: string } | null => {
  ...
  const match = clean.match(/^(Travel to Destination|Return Journey|Hotel Check-in|Hotel|Overnight Stay|Breakfast|Morning|Lunch|Afternoon|Evening|Dinner|Free Time \/ Shopping)(?:\s*\(([^)]+)\))?:?$/i);
  if (!match) return null;
  return { label: match[1], time: match[2] };
};

const parseTravelContentLine = (line: string): { mode?: string; duration?: string; distance?: string } | null => {
  const clean = stripMarkdown(line).replace(/^[🚖🚗🚶🚌🚲🛺\s]+/, "").trim();
  if (!/(auto|rickshaw|cab|car|taxi|bike|bus|walking|walk)/i.test(clean)) return null;
  const parts = clean.split("•").map((part) => part.trim()).filter(Boolean);
  if (parts.length < 2) return null;
  const duration = parts.find((part) => /\b(min|mins|hour|hours|hr|hrs)\b/i.test(part));
  const distance = parts.find((part) => /\b(km|m)\b/i.test(part));
...
export const AIChat: React.FC = () => {
  ...
  const activeLoadingStage = chatProgressStage || loadingStages[loadingStageIndex];
  const activeLoadingStageIndex = Math.max(0, loadingStages.indexOf(activeLoadingStage));
  ...
  }, [isLoadingChat, loadingStages.length]);
                ...
                <div key={idx} className={`flex gap-3 text-left ${isUser ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className="space-y-3.5 max-w-[85%] sm:max-w-[78%]">
                    ...
                    >
                      ...
                      {isOfflineReply && (
                        <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-left text-[11px] font-semibold text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/25 dark:text-amber-300">
                          <div className="flex items-start gap-2">
                            ...
                            <span>Backend unavailable. This is a demo fallback, so live AI, hotel, restaurant, and calendar data may be missing.</span>
                          </div>
                          {msg.retryQuestion && (
                            <button
                              onClick={() => handleSend(msg.retryQuestion)}
                              className="mt-2 inline-flex items-center gap-1.5 rounded-lg border border-amber-300 bg-white px-2.5 py-1 text-[10px] font-extrabold text-amber-800 transition-colors hover:bg-amber-100 dark:border-amber-800 dark:bg-amber-950/40 dark:hover:bg-amber-900/40"
                            >
                      ...
                      {/* Message Content */}
                      {!isUser && (msg.imageUrls?.length || msg.imageUrl) && !msg.tripCard && !msg.nearbyResult && (
                        <div className="mb-3 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900">
                          {msg.imageUrls && msg.imageUrls.length > 1 ? (
                            <div className="py-2">
                              <ImageSlider images={msg.imageUrls} />
                            </div>
                          ) : (
                            <img
                              src={msg.imageUrls?.[0] || msg.imageUrl}
                              alt="RAG destination"
                              className="h-40 w-full object-contain bg-slate-100 dark:bg-slate-900 sm:h-48"
                              loading="lazy"
                              decoding="async"
                            />
                          )}
...
{!isUser && msg.nearbyResult && !msg.tripCard ? (
  ...
  renderStructuredMessage(msg.text)
                        ...
                        <div className="mt-3.5 border-t border-slate-100 pt-3 text-left dark:border-slate-800/80">
                          ...
                          <div className="h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                            <div
                              ...
                              style={{ width: `${Math.min(100, Math.max(0, msg.questionnaire.progress))}%` }}
                            />
                          </div>
                        </div>
                      )}
        ...
        {isLoadingChat && (
          <ChatLoadingIndicator
            ...
            stageNumber={(activeLoadingStageIndex >= 0 ? activeLoadingStageIndex : loadingStageIndex) + 1}
      ...
      <div className="bg-white dark:bg-[#111827] border-t border-slate-200/60 dark:border-slate-800/60 p-4 flex-shrink-0">
        <div className="max-w-3xl mx-auto space-y-3">
          ...
          {(() => {
            const lastMsg = chatMessages[chatMessages.length - 1];
            const isQuestionnaireMessage = !!lastMsg?.questionnaire;
            const isDateQuestion = isQuestionnaireMessage && lastMsg && lastMsg.role === "assistant" && (
              ...
              lastMsg.text.toLowerCase().includes("check-in") ||
              lastMsg.text.toLowerCase().includes("check-out") ||
              lastMsg.text.toLowerCase().includes("checkin") ||
              lastMsg.text.toLowerCase().includes("checkout")
            );
          ...
          {(() => {
            const lastMsg = chatMessages[chatMessages.length - 1];
            const isQuestionnaireMessage = !!lastMsg?.questionnaire;
            const isDaysQuestion = isQuestionnaireMessage && lastMsg && lastMsg.role === "assistant" && (
          ...
          {/* Dynamic Travel Mode Selector */}
          {(() => {
            const lastMsg = chatMessages[chatMessages.length - 1];
            const isQuestionnaireMessage = !!lastMsg?.questionnaire;
            const isTravelModeQuestion = isQuestionnaireMessage && lastMsg && lastMsg.role === "assistant" && (
              lastMsg.text.toLowerCase().includes("travel mode") ||
              lastMsg.text.toLowerCase().includes("mode of travel") ||
              lastMsg.text.toLowerCase().includes("how would you like to travel") ||
              lastMsg.text.toLowerCase().includes("preferred mode of travel")
            );
            
            if (!isTravelModeQuestion) return null;

            const travelModes = ["Flight", "Train", "Bus", "Car"];

            return (
              <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-teal-50/40 dark:bg-teal-950/15 border border-teal-100/50 dark:border-teal-900/30 text-left animate-fadeIn mb-2">
                <div className="flex items-center gap-2">
                  <Plane className="w-4 h-4 text-teal-600" />
                  <span className="text-[11px] font-bold text-teal-700 dark:text-teal-400">Select Travel Mode:</span>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${
                        selectedTravelMode === mode
                          ...
                          : "bg-white dark:bg-[#1f2937] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
                      }`}
                    >
                      {mode}
                    </button>
          ...
          {(() => {
            const lastMsg = chatMessages[chatMessages.length - 1];
            const isQuestionnaireMessage = !!lastMsg?.questionnaire;
            const isTravelersQuestion = isQuestionnaireMessage && lastMsg && lastMsg.role === "assistant" && (
