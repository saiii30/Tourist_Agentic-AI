...
export const FloatingAICopilot: React.FC<FloatingAICopilotProps> = ({ isOpen, onClose, onApplySuccess }) => {
  ...
  const handleSendPrompt = async (textText?: string) => {
    ...
    if (lowerInput.includes("low") || lowerInput.includes("reduce")) {
      setPreview({
        title: "Adjust Budget Category",
        description: "Change itinerary budget settings from Moderate to Low Tier.",
        diffText: "💰 Budget Tier: Moderate ➔ Low (Est. Cost: ₹8,450 ➔ ₹4,000)",
        effect: () => {
