...
interface AddToItineraryDialogProps {
  isOpen: boolean;
  onClose: () => void;
  item: { type: 'hotel' | 'restaurant' | 'attraction', id: string, name: string, location: string, rating: number, image: string, googlePhotoName?: string | null } | null;
  durationDays: number;
...
export const AddToItineraryDialog: React.FC<AddToItineraryDialogProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      setSelectedDay(1);
      setSelectedTime(item?.type === "hotel" ? "09:00 PM" : "01:00 PM");
    }
  }, [isOpen, item]);
