...
import { Clock, MapPin, Tag, Star, Heart, Trash2, ExternalLink, RefreshCw, Car, Route } from "lucide-react";
import type { Activity } from "../../context/TravelPlannerContext";
import { CrowdDensityWidget } from "./CrowdDensityWidget";

const isUnsplashImage = (value?: string | null): boolean => Boolean(value && /unsplash\.com/i.test(value));

const getImageSrc = (activity: Activity) => {
  if (activity.image && !isUnsplashImage(activity.image)) return activity.image;
  const label = `${activity.title} ${activity.location}`.toLowerCase();
  if (label.includes("temple") || label.includes("mandir") || label.includes("shrine")) {
    return "https://upload.wikimedia.org/wikipedia/commons/thumb/7/7d/Temple_of_India.jpg/400px-Temple_of_India.jpg";
  }
  if (label.includes("beach") || label.includes("sea") || label.includes("coast")) {
    return "https://upload.wikimedia.org/wikipedia/commons/f/fe/Calangute_Beach_Goa.jpg";
  }
  if (label.includes("fort") || label.includes("palace") || label.includes("museum") || label.includes("monument")) {
    return "https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/Tourism_in_India.jpg/400px-Tourism_in_India.jpg";
  }
  if (label.includes("park") || label.includes("garden") || label.includes("lake") || label.includes("waterfall")) {
  ...
  return (
    <div className="group relative flex flex-col md:flex-row items-start gap-4 p-4 bg-white dark:bg-[#1e293b]/60 dark:hover:bg-[#1e293b]/80 border border-slate-200/60 dark:border-slate-800/60 rounded-2xl hover:shadow-md hover:border-slate-350 dark:hover:border-slate-700 transition-all duration-200">
      ...
      <div className="flex-1 min-w-0 space-y-1.5 text-left">
        ...
        {activity.travel && (activity.travel.mode || activity.travel.duration || activity.travel.distance) && (
          <div className="flex flex-wrap items-center gap-3.5 text-[10.5px] font-bold text-teal-650 dark:text-teal-400 pt-1 border-t border-slate-100 dark:border-slate-800/60 mt-1">
            {activity.travel.mode && (
              <div className="flex items-center gap-1">
                <Car className="w-3.5 h-3.5 text-teal-500" />
                <span>{activity.travel.mode}</span>
              </div>
            )}
            ...
            {activity.travel.distance && (
              <div className="flex items-center gap-1">
                <Route className="w-3.5 h-3.5 text-teal-500" />
                <span>{activity.travel.distance}</span>
              </div>
            )}
          </div>
        )}

        {/* AI Crowd Prediction Meter */}
